...
## Minimum and maximum value of the counter.
@export var min_max_value: Vector2i = Vector2i(2, 7)
## Pen treadmill that this object is assigned to, should be of class [Treadmill].
@export var treadmill: Treadmill
## Defines whether odd numbers in the counter should be colored red.
@export var odd_number_counter: bool = false
## Whether to stop counting when the counter goes beyond the maximum value or start over from the beginning.
@export var cycle: bool = true
## Whether to play sound effects when the counter's value changes.
@export var petal_sounds: bool = false
## Whether to save the data of this counter to the save file.
@export var save_petals: bool = false

## [Sprite3D] object for the counter's sprite.
...
func _add_number(backward: bool) -> void:
	current_value += is_backwards(backward)
	
	if cycle:
		current_value = wrapi(current_value, min_max_value.x, min_max_value.y)
	else:
		current_value = clamp(current_value, min_max_value.x, min_max_value.y)
...
## Returns a value according to the direction the treadmill is moving to.
func is_backwards(backward: bool) -> int:
	if backward:
		return -1
	else:
		return 1
